...
query_file = open("/share/project/peitian/Data/Adon/NQ/queries.openai.tsv", "w")
qrel_file = open("/share/project/peitian/Data/Adon/NQ/qrels.openai.tsv", "w")

query_idx = 0
for i, queries in enumerate(all_queries):
    for j, query in enumerate(queries):
        query = query.lower().replace("?", "")
        query_line = "\t".join([str(query_idx), query]) + "\n"
